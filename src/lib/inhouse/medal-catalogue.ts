...
export const MEDAL_CATEGORIES: MedalCategory[] = [
  ...
  },
  {
    ...
    tiers: [
      ...
      { id: 'roshanbert', label: 'Roshanbert', description: 'Top 2 w dobijaniu Roshana', place: 2, icon: 'swords' },
  ...
  {
    id: 'first-blood-deaths',
    label: 'Śmierci od First blooda',
    stat: 'firstBloodDeaths',
    direction: 'highest',
    aggregate: 'sum',
    availability: 'derived',
    parseGated: true,
    note:
      "NOT `firstblood_claimed` — that flags who GOT the first blood. The victim has to be resolved from the killer's `kills_log` against the match's `first_blood_time`.",
    tiers: [
      { id: 'pan-keczup', label: 'Pan Keczup', description: 'Najczęściej ginie od First blood', place: 1, icon: 'target' },
      { id: 'zlamany-psychicznie', label: 'Złamany psychicznie', description: 'Top 2 śmierci od First blooda', place: 2, icon: 'target' },
      { id: 'nokaut-na-pierwszej-randce', label: 'Nokaut na pierwszej randce', description: 'Top 3 śmierci od First blooda', place: 3, icon: 'target' },
    ],
  },
  {
    id: 'lifesteal',
    ...
    parseGated: true,
    note:
      'OpenDota exposes no lifesteal or spell-lifesteal self-heal figure. `hero_healing` is healing given to others and is a different medal. These three cannot be awarded without another data source.',
    tiers: [
