  ...
  return createPortal(
    ...
    >
      <div
        ...
        tabIndex={-1}
        className="relative w-full max-w-4xl my-auto rounded-2xl border border-white/10 bg-zinc-900/95
                   backdrop-blur-md shadow-2xl outline-none"
      >
        <button
          onClick={onClose}
          aria-label="Zamknij"
          className="absolute right-4 top-4 z-10 rounded-lg p-1.5 text-slate-400 hover:text-white
        ...
        <div className="px-6 pt-6 pb-2 sm:px-8 sm:pt-8">
          ...
          <p className="text-slate-400 text-sm mt-1">
            {lobbyName ? (
              <>
                Lobby nazywa się <span className="text-white font-bold">{lobbyName}</span>. Wejdź
                ręcznie albo daj się zaprosić.
              </>
            ) : (
              'Wejdź ręcznie albo daj się zaprosić.'
            )}
          </p>
        </div>
...
function PasswordBox({ password }: { password: string | null }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-[#E7000B]/30 bg-[#E7000B]/10 px-4 py-3">
      <KeyRound className="w-4 h-4 shrink-0 text-[#f87171]" />
      <div className="min-w-0">
        <div className="text-[10px] uppercase tracking-[0.2em] text-slate-400">Hasło</div>
        <div className="text-xl font-black tracking-[0.18em] text-white truncate">{password}</div>
      </div>
      <button
        onClick={copy}
        className="ml-auto shrink-0 inline-flex items-center gap-1.5 rounded-md bg-white/10 hover:bg-white/20
                   px-3 py-2 text-xs font-semibold text-slate-200 transition-colors"
      >
